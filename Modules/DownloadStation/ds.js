...
DownloadStation.prototype.getInfo = function(){
    ...
    return new Promise(function(resolve, reject) {
        ...
        if (eligible.Success) {
            axios.get(dsSetting.URI + "/webapi/DownloadStation/info.cgi", { params: { api: 'SYNO.DownloadStation.Task', version: 1, method: 'getinfo', _sid: dsSetting.server.token }})
            .then(function(resp){
                var content = resp.data;
                if (content.success) {resolve(content)}
                else {reject({"Success": false, "Message": "Une erreur"});}
            }).catch(function(){reject({"Success": false, "Message": "Error Occured"});});
        }
         else {
            reject({
                "Success": false,
...
DownloadStation.prototype.getTasks = function(offset) {
    ...
    return new Promise(function(resolve, reject) {
        ...
        if (eligible.Success) {
            axios.get(dsSetting.URI + "/webapi/DownloadStation/task.cgi", { params: { api: 'SYNO.DownloadStation.Task', version: 1, method: 'list', offset: offset, additional: 'detail,file,transfer', _sid: dsSetting.server.token }})
            .then(function(resp){
                var content = resp.data;
                if (dsSetting.server.debug == true && content && content.data) {
                ...
                if (content.success) {
                    ...
                    if (dsSetting.server.debug == true) {console.log(content);}
                    reject({"Success": false, "Message": "Une erreur"});
                }
            }).catch(function(){
                reject({"Success": false, "Message": "Error Occured"});
            });
        } else {
